...
def divide_data(dir_data,dir_label):
    for (root_d, dirs_d, files_d), (root_l, dirs_l, files_l) in zip(os.walk(dir_data), os.walk(dir_label)):
        ...
        num=len(pairs)
        train=pairs[:int(num*0.5)]
        cross=pairs[int(num*0.5):int(num*0.75)]
        test=pairs[int(num*0.75):]
...
def train_and_save_network():
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(n_epochs):
            ...
            current_loss=loss.eval(feed_dict={X: X_batch, y: y_batch})
            ...
            avg_cross_acc = sum(cross_acc.values()) / len(cross_acc)
            #print("epoch:", epoch, "Cross Accuracy:", cross_acc, "\n average is :", avg_acc)
            print("epoch:", epoch, "training Accuracy:",avg_training_acc,"Cross Accuracy:", avg_cross_acc,"Loss:", current_loss)
            if avg_cross_acc > best_acc:
                best_acc = avg_cross_acc
